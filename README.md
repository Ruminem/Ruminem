...
**[cmake-link-explorer](https://github.com/Ruminem/cmake-link-explorer)** —
CMake 프로젝트에서 무엇이 무엇을 링크하는지 양방향으로 본다. 특히 **누가 이 타겟을
링크하는지**. `CMakeLists.txt`를 파싱하지 않고 CMake의 File API를 읽으며, 링커 맵을
타겟 트리에 붙여 크기를 같은 줄에서 보여준다.

**[vscode-neon-glow](https://github.com/Ruminem/vscode-neon-glow)** —
문법 강조에 네온 글로우를 입힌다. 팔레트를 박아두지 않고 **지금 쓰는 테마의 토큰
색에서 글로우를 유도**하므로 어떤 색 테마와도 맞는다.

둘 다 순수 JavaScript고 빌드 단계도 의존성도 없다.
